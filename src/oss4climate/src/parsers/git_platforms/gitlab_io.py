...
def _web_get(
    ...
    if with_headers and GitlabScraper(cache_lifetime=cache_lifetime).is_relevant_url(
        url, include_self_hosted=False
    ):
...
class GitlabScraper(_GPScraper):
        ...
        **kwargs,
        ...
        if include_self_hosted:
            if url.startswith(GITLAB_ANY_URL_PREFIX):
                ...
                try:
                    self.fetch_repository_language_details(url)
                    return True
                except Exception:
    ...
    def fetch_project_details(
        ...
        (
            readme,
            readme_type,
        ) = self.fetch_repository_readme(
            repo_id,
            fail_on_issue=fail_on_issue,
        )
        raw_languages = self.fetch_repository_language_details(repo_id)
        ...
        if isinstance(fork_details, dict):
            forked_from = _get_from_dict_with_default(
                fork_details, "namespace", {}
            ).get("web_url")
        else:
            forked_from = None
        if "updated_at" in r:
    ...
    def fetch_repositories_in_group(
        ...
        """Convenience pointer to 'fetch_repositories_in_organisation' as Gitlab has
